...
void cgCanvas::drawPoly(int polyID)
{
	...
	for (int i = 0; i < n; i++) {
		...
		float content[3] = { coord.x, coord.y, 1 };
		m = Matrix(3, 1, content);
	...
	VYmax = height;*/
	for (int i = 0; i < n; i++) {
		float content[3] = { outx[i], outy[i], 1 };
		m = Matrix(3, 1, content);
		// let's normalize !
...
void cgCanvas::rotate (float degrees)
{
	...
	const float RO[9] = { cos(theta), -sin(theta), 0,
